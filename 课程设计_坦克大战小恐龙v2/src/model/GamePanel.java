...
public class GamePanel extends JFrame{
    ...
    ArrayList<Enemy> botlist = new ArrayList<Enemy>();          // 批量创建恐龙
    ArrayList<Bullet> removelist = new ArrayList<Bullet>();     // 删除子弹和恐龙
    ArrayList<Tank> playerlist = new ArrayList<Tank>();
    ArrayList<Wall>  grasslist = new ArrayList<Wall>();

    PlayerTank playerTank = new PlayerTank(this, playerimg[3], 100, 700, playerimg[0],
            playerimg[1], playerimg[2], playerimg[3]);    // Player one
    PlayerTank_two playerTank_two = new PlayerTank_two(this, playerimg_two[3], 100, 400, playerimg_two[0],
            playerimg_two[1], playerimg_two[2], playerimg_two[3]);    // Player one
    //Enemy  tank
//    Enemy bot = new Enemy(this, enemy_tank_img[1], 100,200, enemy_tank_img[0], enemy_tank_img[1],
//            enemy_tank_img[2], enemy_tank_img[3]);
    // Enemy dinosaur
    Enemy bot = new Enemy(this, dinosaur[0], 740, 100, dinosaur[0], dinosaur[1], dinosaur[2], dinosaur[3]);
    ...
    public void lunch(){
        ...
        // add well
        for(int i = 0; i<39 ;i++){
            grasslist.add(new Wall(this, Constant.grass_Img, 600, 20+i*20));
            grasslist.add(new Wall(this, Constant.grass_Img, 620, 20+i*20));
        }
        ...
        while(true){
            ...
            if(cnt % 100 == 1 && enemy_cnt <= 4){  // 每重绘100次添加一次敌方坦克
                ...
                int xnum = 800 + random.nextInt(100);
                int ynum = random.nextInt(800);
                // 横坐标
                botlist.add(new Enemy(this, Constant.dinosaur_Img, xnum, ynum, Constant.dinosaur_Img,
...
//        System.out.println(bulletList.size());
        ...
        if(state == 0){
            ...
            gImage.setColor(Color.GREEN);
            gImage.drawString("剩余小恐龙的数量："+botlist.size() , 40, 70);
            gImage.setFont(new Font("仿宋",Font.BOLD, 35));
            gImage.drawString("游戏开始" , 300, 100);
            if(state == 1){
                gImage.drawString("单人游戏" , 450, 100);
            }else{
                gImage.drawString("双人游戏" , 450, 100);
            }

            // 绘制 playerone
            for(Tank player: playerlist){
                player.paintSelf(gImage);  // 多个玩家
            }

            // 循环子弹列表 --> new
//            System.out.println(bulletList.size());// 打印长度
            for(Bullet bullet: bulletList){
                bullet.paintSelf(gImage); // 画列表中的每个元素  在玩家列表中添加空格键盘事件 调用这个东西
            }

            bulletList.removeAll(removelist); // 遍历完removelist后删除remove中的元素 子弹先放removelist中
        ...
        public void keyPressed(KeyEvent e){
            ...
            switch (key){
                ...
                case KeyEvent.VK_ENTER:
                    state = state_tmp;
                    playerlist.add(playerTank); // 添加玩家1
                    if(state == 2){
                        playerlist.add(playerTank_two);  // 状态是2时 添加玩家2
                    }
                    break;
                default:
                    playerTank.keyPressed(e);
                    playerTank_two.keyPressed(e);
            }
        }
        // 键盘松开的方法
        @Override
